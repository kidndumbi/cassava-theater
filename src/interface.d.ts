...
export interface LanguageLearningAPI {
  ...
  updateExerciseStats: (key: string, isCorrect: boolean, snapshot?: { userAnswer: string; correctAnswer: string; nativeText: string; practiceMode?: 'arrange-words' | 'fill-in-missing' | 'spell-the-blanks' | 'conversation'; options?: string[] }) => Promise<{ success: boolean; error?: string }>;
