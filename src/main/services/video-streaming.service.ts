...
function getVideoPath(url: URL): string {
  ...
  try {
    ...
    if (error instanceof URIError) {
      ...
      throw new Error(
        "Bad Request: Invalid 'path' parameter. The path contains characters that cannot be decoded.",
      );
    } else {
      throw error;
    }
...
function handleConversionRequest(
  ...
  command
    .on("error", (err) => {
      console.error("handleConversionRequest() FFmpeg error:", err);
      if (!res.headersSent) {
...
function handleDirectStreamRequest(
  videoPath: string,
  req: IncomingMessage,
  res: ServerResponse,
) {
