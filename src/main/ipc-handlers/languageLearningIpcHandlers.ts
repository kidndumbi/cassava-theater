...
export const languageLearningIpcHandlers = () => {
  ...
  ipcMain.on(
    "language-learning-state-update",
    (_event, newState: LanguageLearningState) => {
  ...
  ipcMain.handle(
    LanguageLearningIPCChannels.GET_EXERCISE,
  ...
  ipcMain.handle(
    LanguageLearningIPCChannels.GET_EXERCISES_BY_VIDEO,
    async (_event, videoFilePath: string) => {
      try {
        const exercises =
          await getLanguageLearningExercisesByVideo(videoFilePath);
  ...
  // Delete exercise
  ipcMain.handle(
    LanguageLearningIPCChannels.DELETE_EXERCISE,
    async (_event, key: string) => {
      try {
  ...
  // Update exercise
  ipcMain.handle(
    LanguageLearningIPCChannels.UPDATE_EXERCISE,
    async (
      _event,
      key: string,
...
export function updateLanguageLearningState(
  newState: Partial<LanguageLearningState>,
) {
  currentState = { ...currentState, ...newState };
}
