...
contextBridge.exposeInMainWorld("languageLearningAPI", {
  ...
  updateExerciseStats: (key: string, isCorrect: boolean, snapshot?: { userAnswer: string; correctAnswer: string; nativeText: string; practiceMode?: 'arrange-words' | 'fill-in-missing' | 'spell-the-blanks' }) => {
  ...
  getExerciseLogs: (key: string) => {
    return ipcRenderer.invoke(LanguageLearningIPCChannels.GET_EXERCISE_LOGS, key);
  },
});
